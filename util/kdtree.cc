...
namespace util_kdtree {
    ...
    T KDTree<T, K>::FindNearestPointNormal(
        ...
        LOG(INFO) << "Value " << value_.index;
        LOG(INFO) << (value_.point - point).squaredNorm();
        if ((value_.point - point).squaredNorm() < threshold * threshold) {
            ...
            *neighbor_node = value_;
            ...
            LOG(INFO) << "Dist for node " << current_best_dist << ", " << value_.index;
    ...
    template
    struct KDNodeValue<double, 3>;

    template
    class KDTree<double, 3>;

    // Explicit instantiation of KDTree class for 3D float values.
    template
    struct KDNodeValue<float, 3>;

    template
    class KDTree<float, 3>;

    // Explicit instantiation of KDTree class for 2D double values.
    template
    struct KDNodeValue<double, 2>;

    template
    class KDTree<double, 2>;

    // Explicit instantiation of KDTree class for 2D float values.
    template
    struct KDNodeValue<float, 2>;

    template
    class KDTree<float, 2>;
