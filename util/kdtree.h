...
template <typename T, unsigned int K> struct KDNodeValue {
...
template <typename T, unsigned int K> class KDTree {
 public:
  // Default constructor: Creates an empty KDTree with uninitialized root node.
  KDTree()
      : splitting_dimension_(0), left_tree_(NULL), right_tree_(NULL),
        parent_tree_(NULL) {}
  ...
  T FindNearestPoint(const Eigen::Matrix<T, K, 1>& point, const T& threshold,
                     KDNodeValue<T, K> *neighbor_node) const {
  ...
  void FindNeighborPoints(const Eigen::Matrix<T, K, 1>& point,
    ...
    if (current_dist < threshold)
      neighbor_points->push_back(value_);
...
private:
  ...
  struct VectorComparator {
    ...
    explicit VectorComparator(int dimension)
        : comparator_dimension(dimension) {}
  ...
  int GetSplittingPlane(const std::vector<KDNodeValue<T, K>>& values) {
    ...
    // Chose the splitting plane along the dimension that has the greatest
    // spread, as indicated by the standard deviation along that dimension.
