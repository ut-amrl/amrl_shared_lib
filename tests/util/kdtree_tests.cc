...
TEST(KDTree, FindNearestPoint) {
  ...
  values.emplace_back(
      KDNodeValue<double, 2>(Eigen::Vector2d(1, 2), Eigen::Vector2d(3, 4), 1));
  values.emplace_back(
      KDNodeValue<double, 2>(Eigen::Vector2d(5, 6), Eigen::Vector2d(7, 8), 2));
  values.emplace_back(KDNodeValue<double, 2>(Eigen::Vector2d(9, 10),
                                             Eigen::Vector2d(11, 12), 3));

  KDTree<double, 2> kd_tree(values);

  KDNodeValue<double, 2> return_val;
  double nearest_point_dist_1 =
      kd_tree.FindNearestPoint(Eigen::Vector2d(1, 1), 3, &return_val);
  ...
  double nearest_point_dist_2 =
      kd_tree.FindNearestPoint(Eigen::Vector2d(2, 10), 6, &return_val);
  ...
  double nearest_point_dist_3 =
      kd_tree.FindNearestPoint(Eigen::Vector2d(14, 22), 14, &return_val);
...
TEST(KDTree, FindNeighborPoints) {
  ...
  // This should return the second 2 points, but we don't know what order
  // they'll be returned in, so we have to verify that one of the possible two
...
TEST(KDTree, FindNearestPointNormal) {
  ...
  values.emplace_back(
      KDNodeValue<double, 2>(Eigen::Vector2d(1, 2), Eigen::Vector2d(3, 4), 1));
  values.emplace_back(
      KDNodeValue<double, 2>(Eigen::Vector2d(5, 6), Eigen::Vector2d(0, 1), 2));
  values.emplace_back(KDNodeValue<double, 2>(Eigen::Vector2d(9, 10),
                                             Eigen::Vector2d(11, 12), 3));
  ...
  KDNodeValue<double, 2> return_val;
  double nearest_point_dist_1 =
      kd_tree.FindNearestPointNormal(Eigen::Vector2d(-10, 6), 20, &return_val);
  ASSERT_EQ(values[1].index, return_val.index);
  ASSERT_EQ(values[1].point, return_val.point);
  ASSERT_EQ(values[1].normal, return_val.normal);
